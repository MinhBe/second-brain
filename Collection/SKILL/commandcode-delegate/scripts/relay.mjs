...
function dispatchToCommandCode(opts, brief, run, writeResult, env) {
  ...
  // A grandchild that outlives commandcode and inherited the pipes keeps stdout/stderr open, so
  // "close" never fires and the relay waits forever, writing no result. Once commandcode itself is
