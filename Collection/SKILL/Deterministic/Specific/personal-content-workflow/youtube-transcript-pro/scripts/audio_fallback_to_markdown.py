...
media stream, transcribes it with the local faster-whisper pipeline, and writes the
...
def transcribe_media(
    ...
    if str(model_path).lower().endswith('.bin'):
        raise ValueError('--model accepts a cached faster-whisper model name, not a ggml .bin file')
    pipeline = Path(__file__).resolve().parents[2] / 'audio-transcribe-local' / 'scripts' / 'transcribe.py'
    work = transcript_path.with_suffix('.asr').resolve()
    cmd = [sys.executable, '-X', 'utf8', '-u', '-B', str(pipeline), '--input', str(media_path.resolve()),
        '--output', str(work), '--model', str(model_path), '--language', language,
        '--device', 'cuda' if use_gpu else 'cpu', '--compute-type', 'int8_float16' if use_gpu else 'int8']
    completed = run_command(cmd, workspace)
    if completed.returncode != 0:
        raise RuntimeError(completed.stdout.strip()[-4000:])

    raw = json.loads((work / 'raw.json').read_text(encoding='utf-8'))
    text = '\n'.join(segment['text'] for segment in raw['segments']).strip()
    if not text:
        raise RuntimeError('ASR found no usable speech; inspect the quality report')
...
def main() -> int:
    ...
    if args.model.lower().endswith('.bin'):
        raise SystemExit('--model accepts a cached faster-whisper model name; ggml .bin is unsupported')
