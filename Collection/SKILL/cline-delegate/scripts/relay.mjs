...
function dispatchToCline(opts, brief, run, writeResult) {
  ...
  // A grandchild that outlives cline and inherited the pipes keeps stdout/stderr open, so
  // "close" never fires and the relay waits forever, writing no result. Once cline itself is
