...
function dispatchToCopilot(opts, run, writeResult, onReady) {
  ...
  // A grandchild that outlives copilot and inherited the pipes keeps stdout/stderr open, so
  // "close" never fires and the relay waits forever, writing no result. Once copilot itself is
