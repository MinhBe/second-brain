...
function dispatchToCursor(opts, brief, run, writeResult) {
  ...
  // A grandchild that outlives cursor and inherited the pipes keeps stdout/stderr open, so
  // "close" never fires and the relay waits forever, writing no result. Once cursor itself is
