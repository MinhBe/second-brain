...
Use this skill when the user wants PDFs converted into `.md` or `.txt`, especially batch conversion of language-learning PDFs under `C:\Users\Admin\Documents\Language`.

## Workflow

- Use `pdf_to_text.py` for conversion. It accepts a single PDF or a folder and writes outputs without modifying source PDFs.
- Default input is `C:\Users\Admin\Documents\Language`.
- Default output is `C:\Users\Admin\Documents\Language\_converted_pdf_text`.
...
```powershell
python "C:\Users\Admin\Documents\Collection\Skills\ChangePdfToText\run_convert.py"
```

Convert a specific file or folder:

```powershell
python "C:\Users\Admin\Documents\Collection\Skills\ChangePdfToText\pdf_to_text.py" --input "C:\path\to\file-or-folder" --output "C:\path\to\output"
```
