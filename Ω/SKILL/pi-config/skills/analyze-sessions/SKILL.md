...
description: Analyze past pi agent sessions stored under <pi config>/sessions. Use when the user asks about cost (totals, per project, per model, per day), wants to mine prompting patterns from past prompts, view a specific past session, or search across session transcripts.
---

# Analyze Sessions

Tools for querying past pi sessions. All scripts are stdlib Python 3, no dependencies, and read directly from `<pi config>/sessions/`.
...
All scripts share the same filter vocabulary (see "Shared filters" below). Run them with `python` from anywhere:

```bash
python <pi config>/skills/analyze-sessions/scripts/<script>.py [args]
```
...
python cost.py --since 30d --by project --limit 10

# Cost-per-model (each assistant message credited to its own model)
python cost.py --since 30d --by model

# The 10 most expensive sessions of the last month
python cost.py --since 30d --by session --limit 10

# Cost of one project, all time
python cost.py --cwd /path/to/your/project

# Grand total only
python cost.py --since 30d --by total

# Machine-readable
python cost.py --since 30d --by day --json
```
...
python prompts.py --since 30d

# Tighter cap, one prompt per JSONL line
python prompts.py --since 7d --max-chars 1500 --format jsonl

# One project's prompts
python prompts.py --cwd /path/to/your/project --since 30d

# Prompts that mention a topic
python prompts.py --grep "rate limit" --since 60d
```
...
python show_session.py --latest

# A specific session by id prefix (8 chars is enough)
python show_session.py --session 019e475b

# The most recent session in a project
python show_session.py --latest --cwd /path/to/your/project

# Include subagent transcripts inline below
python show_session.py --session 019e475b --include-subagents-content

# Drop thinking entirely / show fewer chars
python show_session.py --session 019e475b --max-thinking -1 --max-tool-output 800
```
...
python search.py "supabase RLS"

# Only my prompts, last 60 days
python search.py "global instruction" --in user --since 60d

# Regex
python search.py --regex "TODO\\(.+\\)"

# More context per match
python search.py "rate limit" --context 2
```

Each hit prints the session header plus a `python show_session.py --session <id>` line so you can drill in directly.
...
| Question | Command |
|---|---|
| Total cost in the last 7 days | `python cost.py --since 7d --by total` |
| Daily spend trend, last 30 days | `python cost.py --since 30d --by day` |
| Most expensive projects this month | `python cost.py --since 30d --by project --limit 10` |
| Most expensive sessions ever | `python cost.py --by session --limit 10 --until 1d` |
| Cost of one project | `python cost.py --cwd /path/to/proj` |
| Patterns in my prompting | `python prompts.py --since 30d --max-chars 1500` → read the output |
| What did I do yesterday | `python show_session.py --latest --since 1d` |
| Where did the agent struggle | `python cost.py --since 30d --errors-only --by session --limit 10` |
| Find old session about X | `python search.py "X"` |
