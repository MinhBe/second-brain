...
```bash
cp extensions/ask-user-question.ts <pi config>/extensions/
```

Directory extension:

```bash
cp -r extensions/browser <pi config>/extensions/
```

If the copied extension has a `package.json`, install its deps:

```bash
cd <pi config>/extensions/browser
npm install
```
...
```bash
cp -r skills/pdf-reader <pi config>/skills/
```
...
Avoid cloning this repo directly into `<pi config>` unless it is a fresh setup. If you already use pi, copy individual files/folders instead so you don't replace your own config.
...
```bash
python -m venv <pi config>/skills/pdf-reader/.venv
<pi config>/skills/pdf-reader/.venv/bin/pip install -r <pi config>/skills/pdf-reader/requirements.txt
```
