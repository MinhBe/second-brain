 ...
 *
 * Requires:
 *   - SoX installed (Windows: `winget install ChrisBagwell.SoX`; macOS: `brew install sox`)
...
const DEBUG_LOG_PATH = path.join(os.tmpdir(), "dictate-debug.log");
...
// generation) to the OS temp directory (Windows: %TEMP%\\dictate-debug.log;
// macOS/Linux: /tmp/dictate-debug.log).
...
export default function (pi: ExtensionAPI) {
  ...
  const flush = () => {
    ...
    // the clipboard and say so. Windows uses clip.exe; macOS uses pbcopy.
    try {
      const clipboardCmd = process.platform === "win32" ? "clip.exe" : "pbcopy";
      const p = spawn(clipboardCmd, [], { stdio: ["pipe", "ignore", "ignore"] });
      p.stdin.end(text);
    } catch {}
  ...
  const startDictation = (ctx: ExtensionContext) => {
    ...
    // Spawn SoX to capture 16kHz / 16-bit / mono PCM to stdout.
    // macOS/Linux normally provide the `rec` alias. Windows SoX installs
    // `sox.exe`, so we use `sox -d` there. Override with DICTATE_AUDIO_CMD
    // if your binary has a custom path/name.
    const audioCmd = process.env.DICTATE_AUDIO_CMD || (process.platform === "win32" ? "sox" : "rec");
    const audioArgs = [
      "-q", // quiet
      ...
      "--buffer", "512",
      ...(process.platform === "win32" && !process.env.DICTATE_AUDIO_CMD ? ["-d"] : []),
      "-r", "16000",
      "-c", "1",
      "-b", "16",
      "-e", "signed-integer",
    ...
    try {
      proc = spawn(audioCmd, audioArgs, { stdio: ["ignore", "pipe", "pipe"] });
    } catch (e: any) {
      ctx.ui.notify(`Failed to spawn '${audioCmd}'. Install SoX and ensure it is on PATH. Windows: winget install ChrisBagwell.SoX`, "error");
      cleanup();
      return;
    }
    rec = proc;
    ...
    proc.on("error", (err) => {
      ...
      ctx.ui.notify(`${audioCmd} error: ${err.message} (install SoX and ensure it is on PATH)`, "error");
      cleanup();
    });
    ...
    proc.on("exit", (code) => {
      ...
      if (state === "recording" && code !== null && code !== 0) {
        if (activeCtx) {
          activeCtx.ui.notify(`${audioCmd} exited unexpectedly (code ${code})`, "warning");
        }
        cleanup();
      }
    });
