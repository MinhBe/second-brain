...
- **Where text goes:** to whatever input field is focused **when you stop** (never replaces, always appends):
  ...
  - Nothing text-capable focused → transcript is copied to the clipboard (via `clip.exe`, so macOS-only) and a notification says so. A finished dictation is never lost.
...
Or manually: copy `index.ts` to `<pi config>/extensions/dictate/index.ts`.

## One-time setup

```bash
winget install ChrisBagwell.SoX                              # provides `sox` for audio capture
...
- The extension spawns `sox` (sox) capturing 16kHz mono 16-bit PCM to stdout.
...
- **"Failed to spawn 'sox'" / "rec error"** — `winget install ChrisBagwell.SoX` and verify with `which sox`.
...
- **Need lifecycle logs?** Run pi with `DICTATE_DEBUG=1` — the extension appends timestamped events (key hits, toggles, WebSocket open/error/close with their session generation) to `%TEMP%\dictate-debug.log`.
